	...
	action = ""
		...
		switch (action.type) {
			...
			case "UPDATE_PAIRBONDREL_FULFILLED": {
				...
				const oldPairBondRelIndex = state.pairBondRels.findIndex(
					pairBondRel => pairBondRel._id === newPairBondRel._id
				);
				const newPairBondRels = [
					...state.pairBondRels.slice(0, oldPairBondRelIndex),
					newPairBondRel,
					...state.pairBondRels.slice(oldPairBondRelIndex+1)
				];
